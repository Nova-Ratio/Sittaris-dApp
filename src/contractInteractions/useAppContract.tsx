...
export const callApprove = async (address: string, amount: number) => {
    try {
        ...
        ToastError.fire({
            title:
                "There was an error during the approve process. Please try again.",
        });
        return false;
    }
}
...
export const callPaymentTokenApprove = async (address: string, amount: number) => {
    try {
        const { contractWithSigner, msgSender } = await callPaymentTokenContract();
        const tx = await contractWithSigner.approve(address, amount);
        await tx.wait();
...
export const callBuyTokens = async (amount: number) => {
    try {
        ...
        const price = priceWD / 10 ** decimals;
        const amountToPay = amount * price;        
        const { contractWithSigner, msgSender, publicSaleAddress } = await callSaleContract();
        const checker = await callPaymentTokenAllowance(publicSaleAddress);
        if (checker < amountToPay) {
            await callPaymentTokenApprove(publicSaleAddress, parseToDecimals(amountToPay, decimals));
        }
...
export const callNextDistributionTimestamp = async () => {
    try {
        const { contractWithSigner, msgSender } = await callSaleContract();
        const nextDistributionTimestamp = await contractWithSigner.nextDistributionTime();
...
export const callStake = async (amount: number) => {
    try {        
        const { contractWithSigner, msgSender, stakingAddress} = await callStakingContract();
        ...
        ToastError.fire({
            title:
                "There was an error during the stake process. Please try again.",
        });
        return false;
    }
}
...
export const callUnstake = async (amount: number) => {
    try {
        ...
        ToastError.fire({
            title:
                "There was an error during the unstake process. Please try again.",
        });
        return false;
    }
}
...
export const callClaim = async () => {
    try {
        ...
        ToastError.fire({
            title:
                "There was an error during the claim process. Please try again.",
        });
        return false;
    }
}
...
export const callAPY = async () => {
    try {
        ...
        ToastError.fire({
            title:
                "There was an error during the get apy process. Please try again.",
        });
        return false;
    }
}