...
export const callSitTokenDecimals = async () => {
    try {
        ...
        ToastError.fire({
            title: "There was an error during the tokenDecimals process. Please try again.",
        });
        return false;
    }
}
...
export const callStake = async (amount: number, period: number) => {
    try {
        ...
        const decimals = await callSitTokenDecimals();
        const amountToStake = amount * 10 ** Number(decimals);
        const tx = await contractWithSigner.stake(amountToStake, period);
        await tx.wait();
        return true;
    } catch (error) {
